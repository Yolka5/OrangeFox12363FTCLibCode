...
//        telemetry.addLine("pipeline running");
        ...
        Scalar upperYellow = new Scalar(170, 255, 255);

//        telemetry.addLine("hello");



        Core.inRange(hsvFrame, lowerYellow, upperYellow, blueMask);
    ...
    private MatOfPoint findLargestContour(List<MatOfPoint> contours) {
        ...
        Rect scanArea = new Rect(0, 0, 1280, 500);

        for (MatOfPoint contour : contours) {
            double area = Imgproc.contourArea(contour);
            Moments moments = Imgproc.moments(contour);
            cXtemp = moments.get_m10() / moments.get_m00();
            cYtemp = moments.get_m01() / moments.get_m00();
            if (area > maxArea && scanArea.contains(new Point(cX + 10, cY + 20))) {
                maxArea = area;
