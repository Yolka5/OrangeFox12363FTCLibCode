...
//@Autonomous(group = "drive", name = "NOStrafeAutoRedFarSide")
public class AutoRedFarSide extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        //---------------------------
        // --- put on prop line ---
        TrajectorySequence leftPropStart = drive.trajectorySequenceBuilder(new Pose2d(-38, -60, Math.toRadians(90)))
                .splineTo(new Vector2d(-38, -45), Math.toRadians(130))
                .build();
        TrajectorySequence rightPropStart =  drive.trajectorySequenceBuilder(new Pose2d(-38, -60, Math.toRadians(90)))
                .splineTo(new Vector2d(-32, -45), Math.toRadians(50))
                .build();
        TrajectorySequence midPropStart = drive.trajectorySequenceBuilder(new Pose2d(-38, -60, Math.toRadians(90)))
                .splineTo(new Vector2d(-35, -11), Math.toRadians(90))
                .build();
        // --- take from stack ---
...
//                .build();
        ...
        TrajectorySequence leftPropToBkB = drive.trajectorySequenceBuilder(leftTakeFromStk.end())
                .lineToConstantHeading(new Vector2d(20, -11.5))
                .setReversed(true)
                .splineTo(new Vector2d(38, -30), Math.toRadians(0))
                .setReversed(false)
                .build(); //to back bord
        TrajectorySequence rightPropToBkB = drive.trajectorySequenceBuilder(rightTakeFromStk.end())
                .lineToConstantHeading(new Vector2d(20, -11.5))
                .setReversed(true)
                .splineTo(new Vector2d(38, -41), Math.toRadians(0))
                .setReversed(false)
                .build(); //to back bord
        TrajectorySequence midPropToBkB = drive.trajectorySequenceBuilder(midTakeFromStk.end())
                .lineToConstantHeading(new Vector2d(20, -11.5))
                .setReversed(true)
...
//            telemetry.addData("Distance in Inch", (getDistance(width)));
            telemetry.addData("Rect_Num:", rect);
            telemetry.update();
            drive.setPoseEstimate(new Pose2d(-38, -60, Math.toRadians(90)));
            if (starting){
                switch (rect){
                    case 0:
